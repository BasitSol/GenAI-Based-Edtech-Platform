...
│   ├── baseline.yaml                  # Final deterministic-first configuration
│   ├── advanced.yaml                  # BGE retrieval configuration
│   └── evaluation.yaml                # Datasets, metrics, and quality gates
├── src/
│   ├── ingestion/
│   │   ├── pipeline.py                # Discovery and page extraction
...
│   │   └── pipeline.py
│   ├── indexing/
│   │   ├── embedding_service.py       # BGE-M3 service
...
│   ├── retrieval/
│   │   ├── query_classifier.py
│   │   ├── paper_reference_parser.py
│   │   ├── hybrid_retriever.py
│   │   ├── reranker.py                # BGE cross-encoder
│   │   ├── authority_controller.py
│   │   └── context_builder.py         # Expansion + compression
│   ├── generation/
│   │   ├── prompts.py
│   │   ├── answer_generator.py
...
│   ├── workflows/
│   │   └── rag_graph.py               # Deterministic RAG orchestration
│   ├── api/
│   │   ├── main.py
│   │   └── schemas.py
│   └── core.py
├── evaluation/
│   ├── datasets/
│   │   ├── development.jsonl          # 120 public development records
│   │   └── validation.jsonl           # 50 public validation records
│   ├── ingestion_eval.py
│   ├── retrieval_eval.py
│   ├── answer_eval.py
│   ├── citation_eval.py
...
├── .gitignore                         # Secrets, PDFs, models, indexes, caches, and private sets
├── Dockerfile
├── pytest.ini
├── requirements.txt
└── README.md
