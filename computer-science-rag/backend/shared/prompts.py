...
PROMPT_LIBRARY_VERSION = "enterprise-2.2.0"
...
EXAM_QUESTION, SYLLABUS, EXAMINER_FEEDBACK, and CONVERSATIONAL_TRANSFORM. Use structure and meaning, not isolated keywords.""",
    ...
    "assessment_generation": PromptTemplate(
        "assessment_generation", "1.2.0", "Generate a teacher-reviewable grounded assessment",
        "Generate an original Cambridge-style assessment with a balanced mix of textbook-grounded and past-paper-adapted items. "
        "Textbooks support teaching facts and model answers. Past papers support adapted Cambridge question wording, structure, command words, and cited task anchors; never call an adapted item official. "
        "Use mark-scheme and examiner-report sources only for mark allocation, rubric granularity, answer length, and marking-quality patterns. "
        "For a mock test, assess only the explicitly selected syllabus topics, cover each selected topic, honour the exact requested total marks, and exclude coding whenever the selected scope is theory-only. "
        "Return a complete question, model answer, marks, and marking rubric for each item. The rubric must contain exactly one independently assessable criterion per available mark; do not combine multiple marks into one rubric string. "
        "Do not claim a generated question is an official past-paper question.",
    ),
