...
from backend.module1_rag.retrieval.context_builder import build_context
...
def _graph(retriever: HybridRetriever, maximum_chunks: int | None):
    ...
    def retrieve_candidates(state: RetrievalState) -> dict:
        ...
        arguments = {"result_limit": 24}
        if state.get("document_type"):
            arguments["document_type"] = state["document_type"]
        raw = retriever.retrieve(query, state.get("level"), state.get("exam_year"), **arguments)
    ...
    def check(state: RetrievalState) -> dict:
        ...
        sufficient = bool(state["ranked"]) and (raw["exact_mark_scheme_available"] or overlap >= 0.20 or state.get("document_type") == "QUESTION_PAPER")
...
def retrieve(query: str, level: str | None = None, exam_year: int | None = None, conversation_state: dict | None = None,
             maximum_chunks: int | None = None, retriever: HybridRetriever | None = None,
    ...
    try:
        result = _graph(system, maximum_chunks).invoke({
            ...
            "conversation_state": conversation_state or {}, "document_type": document_type,
        })["result"]
    finally:
        if owns_retriever:
            system.close()
