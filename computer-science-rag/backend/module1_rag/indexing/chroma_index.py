...
class ChromaIndex:
    ...
    def search(self, query: str, k: int, filters: dict | None = None) -> list[dict]:
        ...
        conditions = [{key: value} for key, value in (filters or {}).items()
                      if key in {"document_type", "level"} and value is not None]
        # Chroma accepts one simple equality condition directly, but two or
        # more metadata conditions must be expressed through a single logical
        # operator. This matters for source-specific retrieval lanes such as
        # ``O_LEVEL + QUESTION_PAPER`` used by mock-test generation.
